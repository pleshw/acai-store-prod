class BagBuilder{static _items=[];static get items(){return BagBuilder._items}static set items(arrItems){this._items=arrItems;BagBuilder.resetBagComponent()}static addItem(item){BagBuilder._items.push(item);BagBuilder.resetBagComponent()}static removeItem(item){BagBuilder.removeItemById(item.id)}static removeItemById(itemId){const itemIndex=BagBuilder._items.findIndex(i=>i.id===itemId);if(itemIndex===-1){throw new Error(`Invalid item at ${this.name}.`)}BagBuilder._items.splice(itemIndex,1);BagBuilder.resetBagComponent()}static resetBagComponent(){BagBuilder.resetBagInfoComponent();BagBuilder.resetBagPriceComponent()}static resetBagPriceComponent(){const elementBagTotalPrice=document.getElementById("bagTotalPrice");if(elementBagTotalPrice){elementBagTotalPrice.innerHTML=formatItemPrice(BagBuilder.totalPrice)}}static resetBagInfoComponent(){const elementOffcanvasBagInfo=document.getElementById("offcanvasBagInfo");if(elementOffcanvasBagInfo){elementOffcanvasBagInfo.innerHTML="";BagBuilder._items.forEach(i=>{elementOffcanvasBagInfo.insertAdjacentHTML("beforeend",templateItemOffcanvasBagInfo(i))})}}static get itemsDetails(){return BagBuilder.items.map(i=>i.item)}static get totalPrice(){if(!BagBuilder._items||BagBuilder._items.length<1){return 0}let arrAddonsPrices=BagBuilder.items.map(a=>a.price);return arrAddonsPrices.reduce((acc,next)=>acc+next,0)}static get addons(){if(!this.addons||this.addons.length<1){return 0}let arrAddonsPrices=this.items.map(a=>a.price);return arrAddonsPrices.reduce((acc,next)=>acc+next,0)}static reset(){this.items=[]}static buildAndReset(){const result=this.build();this.reset();return result}}function formatItemPrice(numItemPrice){return numItemPrice.toLocaleString("pt-BR",{style:"currency",currency:"BRL"}).replaceAll(" ","")}function makeInputSearchShowcase(inputId){const inputElement=document.getElementById(inputId);if(!inputElement){throw new Error("Invalid input element.")}inputElement.addEventListener("input",function(){var searchTerm=this.value.trim().toLowerCase().replaceAll(" ","");var arrElementItemShowCase=Array.from(document.querySelectorAll(".item-showcase"));arrElementItemShowCase.forEach(function(itemCardElement){const titleContainsSearchTerm=itemCardElement.querySelector(".item-showcase-name").innerHTML.toLowerCase().replaceAll(" ","").includes(searchTerm);const descriptionContainsSearchTerm=itemCardElement.querySelector(".item-showcase-description").innerHTML.toLowerCase().replaceAll(" ","").includes(searchTerm);if(titleContainsSearchTerm||descriptionContainsSearchTerm){itemCardElement.classList.remove("d-none")}else{itemCardElement.classList.add("d-none")}})});inputElement.addEventListener("input",function(event){if(event.inputType==="deleteContentBackward"||event.inputType==="deleteContentForward"){if(this.value.trim()===""){var elementos=document.querySelectorAll("#elementos > div");elementos.forEach(function(elemento){elemento.classList.remove("d-none")})}}})}function resetModalItemCount(modalId){const spanAmountSelectedItem=document.getElementById(`${modalId}Amount`);if(spanAmountSelectedItem){spanAmountSelectedItem.innerHTML=modalItemSelectItemBuilder.amount}const buttonDecreaseItemCount=document.getElementById(`${modalId}DecreaseItemCount`);const buttonIncreaseItemCount=document.getElementById(`${modalId}IncreaseItemCount`);if(!buttonIncreaseItemCount||!buttonDecreaseItemCount){throw new Error(`Invalid element at ${this.name}`)}if(modalItemSelectItemBuilder.amount===1){buttonDecreaseItemCount.setAttribute("disabled","")}else{buttonDecreaseItemCount.removeAttribute("disabled")}}function resetModalItemPrice(modalId){const spanModalPrice=document.getElementById(`${modalId}Price`);if(!spanModalPrice){throw new Error(`Invalid element at ${this.name}`)}if(modalItemSelectItemBuilder.details&&modalItemSelectItemBuilder.details.price){spanModalPrice.innerHTML=formatItemPrice(modalItemSelectItemBuilder.totalPrice)}}function addOrderDetailsForItem(modalId,itemName){const h1ModalTitle=document.getElementById(`${modalId}Title`);const spanModalPrice=document.getElementById(`${modalId}Price`);if(!modalItemSelectTitle||!spanModalPrice){throw new Error(`Invalid element at ${this.name}`)}modalItemSelectItemBuilder.details=dictItemsByName.get(itemName);h1ModalTitle.innerHTML=itemName+`<small class="fw-lighter user-select-none"><i>${modalItemSelectItemBuilder.details.freeAddons>1?` - Você pode selecionar até ${modalItemSelectItemBuilder.details.freeAddons} adicionais gratuitamente!</i></small>`:``}`;spanModalPrice.innerHTML=formatItemPrice(modalItemSelectItemBuilder.details.price)}function setHTMLProducts(){const divCoposSection1=document.getElementById("coposCustomizaveis");if(!divCoposSection1){throw new Error(`Invalid element at ${this.name}`)}dictItemsByName.forEach((val,key)=>divCoposSection1.insertAdjacentHTML("beforeend",templateStoreItem("modalItemSelect",key,val)))}function resetButtonsMultipleSelect(modalId,elementId,checkedByDefault=false){const inputItemSelect=document.getElementById(`${modalId}AddonsItem${elementId}`);if(!inputItemSelect){throw new Error(`Invalid element at ${this.name}`)}inputItemSelect.checked=checkedByDefault}function multipleSelectCheckboxSetListeners(modalId,addon){const checkboxElement=document.getElementById(`${modalId}AddonsItem${addon.id}`);if(!checkboxElement){throw new Error(`Invalid element at ${this.name}`)}checkboxElement.addEventListener("change",function(){if(this.checked){modalItemSelectItemBuilder.fillings.add(addon)}else{modalItemSelectItemBuilder.fillings.delete(addon)}})}function setHTMLAddonsSections(modalId){const divAddonsSections=document.getElementById(`${modalId}Addons`);if(!divAddonsSections){throw new Error(`Invalid element at ${this.name}`)}dictAddonsByName.forEach((val,itemName)=>{const divCurrentAddonSection=document.getElementById(`${modalId}AddonsSection${val.section}`);if(!divCurrentAddonSection){divAddonsSections.insertAdjacentHTML("beforeend",getTemplateAddonSectionByItemName(modalId,itemName))}const divAddonsContainer=document.getElementById(`${modalId}AddonsSectionItems${val.section}`);if(!divAddonsContainer){throw new Error(`Invalid element at ${this.name}`)}divAddonsContainer.insertAdjacentHTML("beforeend",getAddonTemplateByItemName(modalId,itemName));switch(val.sectionType){case"counter":itemCountRegulatorSetListeners(modalId,val,0,10);break;case"multipleSelect":multipleSelectCheckboxSetListeners(modalId,val);if(val.checked){modalItemSelectItemBuilder.fillings.add(val)}break;default:itemCountRegulatorSetListeners(modalId,val,0,10);break}})}class ItemBuilder{constructor({onInit=()=>{},onAmountChange=newValue=>{}}){this.details=null;this._amount=1;this.fillings=new Set;this.addons=[];this.onAmountChange=onAmountChange.bind(this);onInit.bind(this)()}get amount(){return this._amount}set amount(val){if(val<1){return}this._amount=val;this.onAmountChange(this._amount)}get totalPrice(){if(!this.details&&!this.details.price){throw new Error("Item inválido. Tente recarregar o site.")}let currentItemPrice=this.details.price;currentItemPrice+=this.addonsPrice;return currentItemPrice*this._amount}get addonsPrice(){if(!this.addons||this.addons.length<1){return 0}let arrAddonsPrices=this.addons.map(a=>a.price);if(this.details.freeAddons&&this.details.freeAddons>0){const arrCurrentAddonsPriceSorted=arrAddonsPrices.sort().reverse();arrCurrentAddonsPriceSorted.splice(0,this.details.freeAddons);arrAddonsPrices=arrCurrentAddonsPriceSorted}return arrAddonsPrices.reduce((acc,next)=>acc+next,0)}get addonsPriceIncludeFreeAddons(){if(!this.addons||this.addons.length<1){return 0}let arrAddonsPrices=this.addons.map(a=>a.price);return arrAddonsPrices.reduce((acc,next)=>acc+next,0)}reset(){this.amount=1;this.details=null;this.addons=[];this.fillings=new Set}build(){if(!this.details||this.amount<1){alert("Erro ao inserir item, por favor recarregue a página e tente novamente");throw new Error("Item inválido, por favor recarregue a página e tente novamente.")}const result={id:getItemUniqueId(),details:this.details,amount:this.amount,addons:this.addons,fillings:this.fillings,price:this.totalPrice,addonsPrice:this.addonsPrice,freeAddonsPrice:this.addonsPriceIncludeFreeAddons-this.addonsPrice};return result}buildAndReset(){const result=this.build();this.reset();return result}}function getItemUniqueId(){const randomNumber=Math.random().toString(36).slice(2,11);const timestamp=(new Date).getTime();return`${randomNumber}-${timestamp}`}const defaultItemImage="https://flordejambu.com/wp-content/uploads/2022/05/acai.png";const dictItemsByName=new Map([["300ml",{name:"300ml",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",description:"Copo de 300ml com 3 adicionais grátis",price:11,freeAddons:3}],["400ml",{name:"400ml",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",description:"Copo de 400ml com 3 adicionais grátis",price:15,freeAddons:3}],["500ml",{name:"500ml",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",description:"Copo de 500ml com 4 adicionais grátis",price:20,freeAddons:4}],["700ml",{name:"700ml",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",description:"Copo de 700ml com 4 adicionais grátis",price:26,freeAddons:4}]]);const dictAddonsByName=new Map([["Açaí",{id:"Acai",name:"Açaí",section:"Açaí",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:true,price:0}],["Creme de Ninho",{id:"CremeDeNinho",name:"Creme de Ninho",section:"Cremes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Creme de cupuaçu",{id:"CremeDeCupuaçu",name:"Creme de cupuaçu",section:"Cremes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Creme mix",{id:"CremeMix",name:"Creme mix",section:"Cremes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Creme de Oreo",{id:"CremeDeOreo",name:"Creme de Oreo",section:"Cremes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Creme de tapioca",{id:"CremeDeTapioca",name:"Creme de tapioca",section:"Cremes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Cookies cream",{id:"CookiesCream",name:"Cookies cream",section:"Cremes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Impacto (Chocolate com castanha)",{id:"ImpactoChocolateComCastanha",name:"Impacto",displayName:"Impacto <hr> (Chocolate com castanha)",section:"Sorvetes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Chocolate",{id:"Chocolate",name:"Chocolate",section:"Sorvetes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Napolitano",{id:"Napolitano",name:"Napolitano",section:"Sorvetes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Morango",{id:"Morango",name:"Morango",section:"Sorvetes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Pavê",{id:"Pavê",name:"Pavê",section:"Sorvetes",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"multipleSelect",checked:false,price:0}],["Leite em pó",{id:"LeiteEmPó",name:"Leite em pó",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Amendoim",{id:"Amendoim",name:"Amendoim",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Amendoim triturado",{id:"AmendoimTriturado",name:"Amendoim triturado",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Paçoca",{id:"Paçoca",name:"Paçoca",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Granola",{id:"Granola",name:"Granola",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Choco ball",{id:"ChocoBall",name:"Choco ball",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Choco power",{id:"ChocoPower",name:"Choco power",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Disquete",{id:"Disquete",name:"Disquete",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Farinha láctea",{id:"FarinhaLáctea",name:"Farinha láctea",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Marshmallow",{id:"Marshmallow",name:"Marshmallow",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Ovomaltine",{id:"Ovomaltine",name:"Ovomaltine",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Jujubas",{id:"Jujubas",name:"Jujubas",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Gotas de Chocolate",{id:"GotasDeChocolate",name:"Gotas de Chocolate",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Castanha",{id:"Castanha",name:"Castanha",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Extras",price:.5}],["Chocolate",{id:"Chocolate",name:"Chocolate",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Coberturas",price:.5}],["Morango",{id:"Morango",name:"Morango",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Coberturas",price:.5}],["Kiwi",{id:"Kiwi",name:"Kiwi",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Coberturas",price:.5}],["Leite condensado",{id:"LeiteCondensado",name:"Leite condensado",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Coberturas",price:.5}],["Pasta de ninho",{id:"PastaDeNinho",name:"Pasta de ninho",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Coberturas",price:.5}],["Pasta de avelã",{id:"PastaDeAvelã",name:"Pasta de avelã",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Coberturas",price:.5}],["Banana",{id:"Banana",name:"Banana",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Frutas",price:.5}],["Morango",{id:"Morango",name:"Morango",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Frutas",price:.5}],["Cereja",{id:"Cereja",name:"Cereja",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Frutas",price:.5}],["Kiwi",{id:"Kiwi",name:"Kiwi",img:"https://flordejambu.com/wp-content/uploads/2022/05/acai.png",sectionType:"counter",checked:false,section:"Frutas",price:.5}]]);function templateItemCountRegulator(parentId,elementId,hasBorders=true){return`
  ...
  `}function resetButtonsItemCountRegulator(parentId,addon,newValue,minValue=0,maxValue=10){const spanAmountSelectedItem=document.getElementById(`${parentId}ItemCountRegulatorCounter${addon.id}`);if(spanAmountSelectedItem){spanAmountSelectedItem.innerHTML=newValue.toString()}const buttonDecreaseItemCount=document.getElementById(`${parentId}ItemCountRegulatorDecreaseItemCount${addon.id}`);const buttonIncreaseItemCount=document.getElementById(`${parentId}ItemCountRegulatorIncreaseItemCount${addon.id}`);if(!buttonIncreaseItemCount||!buttonDecreaseItemCount){throw new Error(`Invalid element at ${this.name}.`)}if(newValue===minValue){buttonDecreaseItemCount.setAttribute("disabled","")}else{buttonDecreaseItemCount.removeAttribute("disabled")}if(newValue===maxValue){buttonIncreaseItemCount.setAttribute("disabled","")}else{buttonIncreaseItemCount.removeAttribute("disabled")}}function itemCountRegulatorSetListeners(parentId,addon,minValue=0,maxValue=10){const buttonDecreaseItemCount=document.getElementById(`${parentId}ItemCountRegulatorDecreaseItemCount${addon.id}`);const buttonIncreaseItemCount=document.getElementById(`${parentId}ItemCountRegulatorIncreaseItemCount${addon.id}`);if(!buttonDecreaseItemCount||!buttonIncreaseItemCount){throw new Error(`Invalid element at ${this.name}.`)}buttonDecreaseItemCount.addEventListener("click",()=>{const spanAmountSelectedItem=document.getElementById(`${parentId}ItemCountRegulatorCounter${addon.id}`);let currentItemAmount=+spanAmountSelectedItem.innerHTML;currentItemAmount--;if(currentItemAmount<minValue){currentItemAmount=minValue}resetButtonsItemCountRegulator(parentId,addon,currentItemAmount,minValue,maxValue);const addonIndex=modalItemSelectItemBuilder.addons.findIndex(a=>a.name===addon.id);if(addonIndex===-1){throw new Error(`Invalid element at ${this.name}.`)}modalItemSelectItemBuilder.addons.splice(addonIndex,1);resetModalItemPrice(parentId)});buttonIncreaseItemCount.addEventListener("click",()=>{const spanAmountSelectedItem=document.getElementById(`${parentId}ItemCountRegulatorCounter${addon.id}`);let currentItemAmount=+spanAmountSelectedItem.innerHTML;currentItemAmount++;if(currentItemAmount>maxValue){currentItemAmount=maxValue}resetButtonsItemCountRegulator(parentId,addon,currentItemAmount,minValue,maxValue);modalItemSelectItemBuilder.addons.push(addon);resetModalItemPrice(parentId)})}function setupModalBuildItem(modalId){const modalItemBuild=document.getElementById(modalId);modalItemBuild.addEventListener("show.bs.modal",event=>{dictAddonsByName.forEach((val,key)=>{switch(val.sectionType){case"counter":break;case"multipleSelect":if(val.checked){modalItemSelectItemBuilder.fillings.add(val)}break;default:break}})});modalItemBuild.addEventListener("hidden.bs.modal",()=>{modalItemSelectItemBuilder.reset();dictAddonsByName.forEach((val,key)=>{switch(val.sectionType){case"counter":resetButtonsItemCountRegulator(modalId,val,0);break;case"multipleSelect":resetButtonsMultipleSelect(modalId,val.id,val.checked);break;default:resetButtonsItemCountRegulator(modalId,val,0);break}})})}function templateStoreItem(modalToTriggerId,itemName,itemInfo){return`
  ...
  `}function templateAddonMultipleSelect(parentId,addon){return`
    <div class="col-12 col-lg-4 col-xl-4 py-3 px-2 justify-content-between align-items-center">
      <input type="checkbox" ${addon.checked?"checked":""} class="btn-check" id="${parentId}AddonsItem${addon.id}" autocomplete="off">
      <label class="btn btn-outline-success border border-2 bg-gradient py-3 py-lg-2 py-xl-2 px-3 d-flex align-items-center justify-content-between m-0 rounded-0" for="${parentId}AddonsItem${addon.id}">
        <h4 class="fw-bolder col-6 text-wrap d-block d-lg-none d-xl-none">${addon.displayName??addon.name}</h4>
        <h5 class="fw-bolder col-6 text-wrap d-none d-lg-block d-xl-block">${addon.displayName??addon.name}</h5>

        ${templateAddonImgMultipleSelect(addon.img)}
      </label>
    </div>
  `}function templateAddonImgMultipleSelect(addonImg){if(!addonImg){return""}return`
    <div class="col-4 col-lg-6 col-xl-6 p-lg-1 p-xl-1 d-flex justify-content-center">
  ...
  `}function getDictCountByAddon(arrAddons){const dict=new Map;arrAddons.forEach(item=>{if(dict.has(item.name)){dict.set(item.name,dict.get(item.name)+1)}else{dict.set(item.name,1)}});return dict}function formatDictCountByAddon(amountByAddon){return[...amountByAddon.entries()].reduce((accumulator,[itemName,itemCount],index,array)=>{const comma=index<array.length-1?", ":"";return accumulator+`${itemCount}x ${itemName}${comma}`},"")}function formatFillings(itemFillings){const tmpFillings=Array.from(itemFillings).map(i=>i.name);if(tmpFillings.length===0)return"";if(tmpFillings.length===1)return tmpFillings[0];const allButLast=tmpFillings.slice(0,-1).join(", ");const last=tmpFillings[tmpFillings.length-1];return`${allButLast} e ${last}`}function removeItemFromBagById(itemId){BagBuilder.removeItemById(itemId)}function templateItemOffcanvasBagInfo(item){const amountByAddon=getDictCountByAddon(item.addons);let strItemAddons="";if(item.addons&&item.addons.length>0){strItemAddons=formatDictCountByAddon(amountByAddon)}else{strItemAddons="Sem adicionais."}return`        
    ...
    </li>`}function setInstagramLink(anchorElement){var userAgent=navigator.userAgent||navigator.vendor||window.opera;if(/android/i.test(userAgent)){anchorElement.href="intent://instagram.com/_u/pacaidelivery1/#Intent;package=com.instagram.android;scheme=https;end"}else if(/iPad|iPhone|iPod/.test(userAgent)&&!window.MSStream){anchorElement.href="instagram://user?username=pacaidelivery1"}else{anchorElement.setAttribute("target","_blank");anchorElement.href="https://www.instagram.com/pacaidelivery1"}}function addCurrentItem(){BagBuilder.addItem(modalItemSelectItemBuilder.buildAndReset())}let modalItemSelectItemBuilder=new ItemBuilder({onInit:function(){const buttonDecreaseItemCount=document.getElementById("modalItemSelectDecreaseItemCount");const buttonIncreaseItemCount=document.getElementById("modalItemSelectIncreaseItemCount");buttonDecreaseItemCount.addEventListener("click",()=>this.amount--);buttonIncreaseItemCount.addEventListener("click",()=>this.amount++)},onAmountChange:function(){resetModalItemCount("modalItemSelect");resetModalItemPrice("modalItemSelect")}});function initStore(){document.querySelectorAll(".instagram-link").forEach(el=>setInstagramLink(el));setHTMLProducts();setHTMLAddonsSections("modalItemSelect")}window.addEventListener("load",()=>{setupModalBuildItem("modalItemSelect");makeInputSearchShowcase("inputBuscaDesktop");makeInputSearchShowcase("inputBuscaMobile")});